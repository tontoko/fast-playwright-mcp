...
function updateTools(content) {
  ...
  const startMarkerIndex = content.indexOf(startMarker, toolsHeadingIndex);
  const endMarkerIndex = content.lastIndexOf(endMarker);
  if (
    toolsHeadingIndex === -1 ||
    startMarkerIndex === -1 ||
    endMarkerIndex < startMarkerIndex
  ) {
    throw new Error('README tools heading or generated markers not found');
  }
  const normalizedContent = [
    ...
    startMarker,
    endMarker,
    content.slice(endMarkerIndex + endMarker.length),
  ].join('\n');
